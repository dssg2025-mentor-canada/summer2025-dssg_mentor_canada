...
predictor_cols = ["38_social_assistance",
                  ...
                  "38_food_bank_use"]
target_cols = ["19c_teen_mentor_unmet_access"]
...
print("count of those who wanted a mentor but had unmet access as a teen (ages 12-18):",
      (df['19c_teen_mentor_unmet_access']== 'Yes').sum())
print("\nRaw target class distributions:")

print(df['19c_teen_mentor_unmet_access'].value_counts())

logistic_data = df.copy()

logistic_data['19c_teen_mentor_unmet_access'] = (
    logistic_data['19c_teen_mentor_unmet_access']
    .map({'Yes': 1, 'No': 2})
    .astype('Int64')
)

# recode 19c_teen_mentor_unmet_access to be binary
logistic_data = logistic_data[
    ...
    logistic_data["19c_teen_mentor_unmet_access"].isin([1, 2])
].copy()
...
logistic_data['19c_teen_mentor_unmet_access'].value_counts()

X = logistic_data[predictor_cols].astype('float64')
X = sm.add_constant(X)
y = logistic_data['19c_teen_mentor_unmet_access'].astype('float64')
