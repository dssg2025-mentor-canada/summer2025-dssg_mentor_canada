...
# Regression – Outcome variable: Yearly income
...
#### Only one-hot encode the nominal columns without imputation:
...
```{r}
ohe_unimputed_train <- preprocessing_recipe_ohe_unimputed |>
                     prep() |>
                  bake(new_data = NULL)
```

> Validate post-ohe, column names are the same between `imputed_train` and `ohe_unimputed_train`:

```{r}
identical(ncol(ohe_unimputed_train), ncol(imputed_train))
identical(nrow(ohe_unimputed_train), nrow(imputed_train))
nrow(ohe_unimputed_train)
nrow(youth)
# Outputs TRUE! They are the same!
```
...
# write_csv(imputed_train, "../../dssg-2025-mentor-canada/Data/mean_mode_imputed_train.csv")
...
# write_rds(imputed_train, "../../dssg-2025-mentor-canada/Data/mean_mode_imputed_train.Rds")
```
...
write_feather(imputed_train, "../../dssg-2025-mentor-canada/Data/mean_mode_imputed_train.feather")
